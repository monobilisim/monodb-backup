...
func (d *Dumper) Dump() {
	d.l.Info("PostgreSQL veritabanı yedeklemesi başladı.")
	d.m.Notify("PostgreSQL veritabanı yedeklemesi başladı.", "", "", false)
	for _, db := range d.p.Databases {
		d.dumpSingleDb(db, d.p.BackupDestination)
	}
	d.l.Info("PostgreSQL veritabanı yedeklemesi sona erdi.")
	d.m.Notify("PostgreSQL veritabanı yedeklemesi sona erdi.", "", "", false)
}
